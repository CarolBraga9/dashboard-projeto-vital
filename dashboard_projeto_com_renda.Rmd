...
<!--
  CORREÇÃO: O HTML estático dos botões foi removido.
  Agora, usamos um uiOutput para renderizar os botões dinamicamente a partir do servidor.
-->
```{r, echo=FALSE}
...
# --- ATUALIZAÇÃO: RENDERIZAÇÃO DINÂMICA E TRADUZIDA DOS BOTÕES DE IDIOMA ---
# Os rótulos dos botões agora são traduzidos usando a função tr().
output$lang_buttons_ui <- renderUI({
  # Garante que a função de tradução esteja pronta antes de renderizar
  req(tr)
  ...
  div(class = "lang-buttons",
      # Usa chaves de tradução para os rótulos dos botões
      actionButton(inputId = "lang_pt", label = tr("lang_button_pt"), class = pt_class),
      actionButton(inputId = "lang_en", label = tr("lang_button_en"), class = en_class)
  )
})
```
