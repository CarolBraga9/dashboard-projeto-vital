...
# ===================================================================
# CHUNK DE SETUP NÃO-REATIVO
# Responsável por instalar pacotes, carregar bibliotecas e ler/processar dados.
# ===================================================================
...
# A URL foi corrigida para ser uma string de texto simples, sem formatação Markdown.
url_planilha <- "https://docs.google.com/spreadsheets/d/1NUxmO-_XYut_3AwsdM9uMo9d9bFTHC47JYONnfbYsw8/edit?usp=sharing"

# --- LEITURA DOS DADOS ---
# Em ambientes não-interativos (como o RMarkdown sendo knit no CI), ler do Google Sheets pode falhar.
# Aqui tentamos ler — se falhar, criamos data.frames vazios para permitir knit estático.

safe_read_sheet <- function(url, sheet) {
  tryCatch(
    read_sheet(url, sheet = sheet),
    error = function(e) {
      message(sprintf("Aviso: falha ao ler sheet '%s': %s. Criando tabela vazia.", sheet, e$message))
      return(data.frame())
    }
  )
}
...
# Só rodar transformações espaciais se os dados existem
crs_sirgas2000 <- 4674
if (nrow(geo) > 0) {
...
num_treinados <- if (nrow(profissionais) > 0) profissionais %>% filter(treinamento_realizado == "Sim") %>% nrow() else NA
nota_media <- if (nrow(profissionais) > 0) profissionais %>% filter(!is.na(nota_avaliacao)) %>% summarise(media = mean(nota_avaliacao, na.rm=TRUE)) %>% pull(media) else NA
total_atendidos <- if (nrow(atendimentos) > 0) n_distinct(atendimentos$id_pessoa) else NA
total_atendimentos <- if (nrow(atendimentos) > 0) nrow(atendimentos) else NA

# Pequena função auxiliar para tradução quando translations_df não existir
get_translation_safe <- function(key, lang = "pt") {
  if (exists("translations_df") && nrow(translations_df) > 0 && key %in% translations_df$key) {
    return(ifelse(lang == "pt", translations_df$pt[translations_df$key == key], translations_df$en[translations_df$key == key]))
  }
  return(paste0("[", key, "]"))
}
...
# ===================================================================
# CHUNK DO SERVIDOR SHINY
# Toda a lógica reativa (observers, reactives, render*) foi centralizada
# aqui. Para evitar erro em ambientes não interativos (CI/GitHub Pages),
# executamos o código reativo apenas quando R é interativo.
# ===================================================================

if (interactive()) {
  library(shinyjs) # garantir disponível no server
  useShinyjs()
...
<!-- --- HTML PARA INSERIR O LOGO (CORREÇÃO CRÍTICA) --- -->
...
<!--
  ...
  agora está no chunk `server` no topo do arquivo.
-->
```{r, echo=FALSE}
...
# A lógica foi movida para o chunk 'server'.
# Esta chamada apenas exibe a tabela pré-renderizada.
