...
# --- ABORDAGEM DE INSTALAÇÃO INDIVIDUAL E CORRIGIDA ---
# CORREÇÃO DEFINITIVA: A URL do repositório está agora como uma string de texto simples,
# que é o formato correto para o R. Cada pacote será verificado e instalado
# individualmente para garantir a robustez do processo.

if (!require("flexdashboard")) { install.packages("flexdashboard", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("shiny")) { install.packages("shiny", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("shinyjs")) { install.packages("shinyjs", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("dplyr")) { install.packages("dplyr", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("leaflet")) { install.packages("leaflet", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("sf")) { install.packages("sf", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("geobr")) { install.packages("geobr", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("htmlwidgets")) { install.packages("htmlwidgets", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("viridis")) { install.packages("viridis", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("timevis")) { install.packages("timevis", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("lubridate")) { install.packages("lubridate", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("DT")) { install.packages("DT", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
if (!require("googlesheets4")) { install.packages("googlesheets4", repos = "[https://cloud.r-project.org](https://cloud.r-project.org)") }
...
# --- URL DA PLANILHA (CORRIGIDA) ---
# CORREÇÃO CRÍTICA: A URL da planilha agora é uma string de texto válida.
...
tryCatch({
  ...
  stop("Falha ao ler os dados do Google Sheets. Verifique a URL, os nomes das abas ('geo', 'treinamento', 'atendimentos', 'traducao') e as permissões. Erro: ", e$message)
})
...
<!--
  Usamos um uiOutput para renderizar os botões dinamicamente a partir do servidor.
-->
```{r, echo=FALSE}
...
```{r}
# Renderiza os textos usando as chaves da sua planilha
...
# Usa a chave correta para o título do resumo
output$summary_title <- renderText({ tr("project_summary_title") })

output$summary_table <- renderDT({
  # As descrições das tarefas agora usam as chaves da sua planilha
...
output$gantt_chart <- renderTimevis({
  # As descrições das tarefas aqui também foram corrigidas
  gantt_data_new <- data.frame(
    stringsAsFactors = FALSE,
    id = 1:6,
    description = c(
