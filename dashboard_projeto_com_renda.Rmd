...
.value-box { border-radius: 8px !important; box-shadow: 0 4px 8px rgba(0,0,0,0.08); transition: all 0.3s ease-in-out; border: 1px solid rgba(0,0,0,0.05); }
.value-box:hover { transform: translateY(-5px); box-shadow: 0 8px 16px rgba(0,0,0,0.12); }
...
.value-box .value, .value-box .caption { color: #333; }
.value-box .icon { color: rgba(0,0,0,0.35) !important; }

/* --- LOGO e NAVBAR --- */
.logo-container { position: fixed; top: 0; left: 0; width: 100%; height: 50px; z-index: 1050; pointer-events: none; }
.logo-left { position: absolute; top: 5px; left: 15px; height: 40px; width: auto; pointer-events: auto; }
.navbar-brand { margin-left: 170px !important; }

/* --- GANTT --- */
.timevis-item.completed { background-color: #198754; border-color: #157347; color: white; }
.timevis-item.not-started { background-color: #F57C00; border-color: #e06f00; color: white; }
...
# ===================================================================
# SETUP: instala/carrega pacotes e lê dados (não cria outputs reativos aqui)
# ===================================================================
...
# garante shiny disponível
library(shiny)

# locale (opcional / tentativa)
try(Sys.setlocale("LC_ALL", "pt_BR.UTF-8"), silent = TRUE)

# desautentica google sheets (caso público)
...
# leitura segura: se não conseguir acessar, retorna data.frame vazio
...
# processamento espacial — só se geo tiver as colunas mínimas
crs_sirgas2000 <- 4674
...
# função de tradução não-reativa (fallback)
...
# NÃO definir outputs aqui — isso será feito no bloco server protegido
```

```{r ui_buttons_placeholder, echo=FALSE}
# Chamamos uiOutput() apenas SE shiny estiver disponível e em modo interativo
if (requireNamespace("shiny", quietly = TRUE) && interactive()) {
  shiny::uiOutput("lang_buttons_ui")
} else {
  # placeholder para manter layout em knit estático
  HTML("")
}
```
...
if (interactive()) {
  ...
  tr <- function(key){
    ...
    # fallback para função não-reativa
